...
if (array_key_exists('tipoUtilizador', $_SESSION)) {
    ...
    if ($tipoUtilizador == false || !is_numeric($tipoUtilizador) || !$TipoUtilizadorManager->existeTipoUtilizador($tipoUtilizador) 
            || $TipoUtilizadorManager->getTipoUtilizadorByNome('Empregador')->getId() != $tipoUtilizador
    ...
    <body>
        ...
        <nav id="navegacaoSegundaria">
            <ul>
                ...
                <li><a href="">Historico de Ofertas de Trabalho Finalizadas</a></li>
            ...
            if (count($listOfertasUtilizador) > 0 && count($listOfertas) > 0) {
                foreach ($listOfertas as $obj) {
                    foreach ($listOfertasUtilizador as $obj2) {
                        if (($obj->getId()) == ($obj2->getOferta()) && $dateNow > ($obj2->getDataFim()) && $obj2->getVencedor() == null) {
                            $listCandidatura = $candidaturaManager->getCandidaturaByOferta($obj->getId());
                ...
                ?><h1>Não existe ofertas pendentes</h1><?php
            }
            ?>
        </section>
        <footer id="rodape">
